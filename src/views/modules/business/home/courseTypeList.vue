...
<div class="mod-config">
  <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <courseTypeAdd v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getselectdata"></courseTypeAdd>
    </div>
...
<script>
  import courseTypeAdd from './courseTypeAdd'
  export default {
    data () {
      return {
        uploadVisible: false,
    ...
    methods: {
      ...
      addOrUpdateHandle (id) {
        ...
        this.$nextTick(() => {
          var data = ''
          for (var j = 0; j < this.dataList.length; j++) {
            data += ',' + this.dataList[j].uuid
          }
          if (this.dataList.length > 0) {
            data = data.substr(1)
          }
          this.$refs.addOrUpdate.getDataList(data)
        })
      },
      dataFormSubmit () {
        var content = ''
        ...
        this.$http({
          url: this.$http.adornUrl(`/generator/renderhome/save/hotCoursertype`),
          method: 'post',
          data: this.$http.adornData({
            'hotCourseTypeList': content
          })
        }).then(({data}) => {
          if (data && data.code === 0) {
            this.$message({
