  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        ...
        <el-button v-if="isAuth('business:translatequestion:add')" type="primary" @click="addOrUpdateHandle()">{{$t('button.add')}}</el-button>
        <el-button v-if="isAuth('business:translatequestion:add')" type="success" @click="autoAddHandle()">{{$t('button.auto_add')}}</el-button>
        <el-button v-if="isAuth('business:translatequestion:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">{{$t('button.batch_delete')}}</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
    ...
    <!-- 自动新增 -->
    <auto-add v-if="visibleAuto" ref="autoAdd" @refreshDataList="getDataList"></auto-add>

  </div>
</template>

<script>
  import AddOrUpdate from './translatequestion-add-or-update'
  import AutoAdd from './translatequestion-auto-add'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/generator/translatequestion/list'),
          method: 'get',
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/generator/translatequestion/delete'),
            method: 'post',
