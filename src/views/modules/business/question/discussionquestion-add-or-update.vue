    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="80px">
      ...
      <el-form-item :label="$t('question.question_content')" prop="question">
        <el-input
          type="textarea"
          autosize
          :placeholder="$t('question.question_content')"
      ...
      <el-form-item :label="$t('question.question_right')" prop="answer">
        <el-input
          type="textarea"
          autosize
          :placeholder="$t('question.question_right')"
      ...
      <el-form-item :label="$t('question.question_analysis')" prop="analysis">
        <el-input
          type="textarea"
          autosize
          :placeholder="$t('question.question_analysis')"
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.uuid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/discussionquestion/info/${this.dataForm.uuid}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.subject = data.discussionQuestion.subject
                this.dataForm.question = data.discussionQuestion.question
                this.dataForm.answer = data.discussionQuestion.answer
                this.dataForm.analysis = data.discussionQuestion.analysis
                this.dataForm.score = data.discussionQuestion.score
                this.dataForm.level = data.discussionQuestion.level
                this.dataForm.section = data.discussionQuestion.section
                this.dataForm.deleteFlag = data.discussionQuestion.deleteFlag
                this.dataForm.tenantId = data.discussionQuestion.tenantId
              }
            })
          }else{
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/discussionquestion/${!this.dataForm.uuid ? 'save' : 'update'}`),
              method: 'post',
