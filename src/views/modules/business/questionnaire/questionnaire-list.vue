      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" :content="$t('button.preview')" placement="top">
            <svg class="icon" aria-hidden="true" @click="preview(scope.row.uuid,scope.row.questionnaireName)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 预览
      preview(uuid,questionnaireName){
        this.questionnairedetailVisible = true
        this.$nextTick(()=>{
          this.$refs.questionnairedetail.init(uuid,questionnaireName)
        })
      },
