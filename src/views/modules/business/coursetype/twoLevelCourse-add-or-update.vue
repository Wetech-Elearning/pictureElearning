<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="80px">
      <el-form-item :label="$t('course_type.course_type_name')" prop="parentCourseType">
        <el-select v-model="dataForm.parentCourseType" :placeholder="$t('course_type.course_type_name')" style="width:100%;">
          <el-option
            v-for="item in oneCourseTypeOptions"
            :key="item.uuid"
      ...
      <el-form-item :label="$t('common.name')" prop="courseTypeName">
        <el-input v-model="dataForm.courseTypeName" :placeholder="$t('common.name')"></el-input>
      </el-form-item>
      <el-form-item :label="$t('course_type.two_level_course_type_introduction')" prop="courseTypeIntroduction">
        <el-input v-model="dataForm.courseTypeIntroduction" :placeholder="$t('course_type.two_level_course_type_introduction')"></el-input>
      </el-form-item>
        <el-form-item :label="$t('certificateinfo.certificateinfo_name')" prop="certificateId">
          <custom-select
            style="width:100%;"
            :action="'/generator/certificateinfo/getUnRelatedCertificateInfoEntityList'"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initOneCourseType () {
        this.$http({
          url: this.$http.adornUrl('/generator/coursetype/listOneLevelCourseType'),
          method: 'get',
          params: this.$http.adornParams({})
        }).then(({data}) => {
          if (data && data.code === 0) {
            this.oneCourseTypeOptions = data.data
          } else {
            this.$message.error(data.msg)
          }
        })
